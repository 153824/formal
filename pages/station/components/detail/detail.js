...
Page({
    ...
    function toNext(promise) {
      app.doAjax({
        ...
        success: function(res) {
          promise.then(ret=>{
            wx.navigateTo({
              url: '../../../replying/components/guide/guide?id=' + res.id
            })
          }).catch((err)=>{
            wx.navigateTo({
              url: '../../../replying/components/guide/guide?id=' + res.id
    ...
    app.doAjax({
      ...
      success: function(res) {
        if (res && res.isOld && res.id) {
          ...
          if (oldData) {
            wx.navigateTo({
              url: '../../../replying/replying?pid=' + that.data.evaluationId + '&id=' + res.id
            });
            return;
          }
  ...
  gotodati: function() {
    ...
    wx.navigateTo({
      url: '../sharePaper/sharePaper?id=' + evaluationInfo.id + "&count=" + availableCount + "&name="
